...
const HomeScreen = ({ navigation, route }) => {
  ...
  // Get connected state from parameters or use default false
  ...
  useEffect(() => {
    if (route.params?.constants) {
      ...
      console.log('Sabit değerler güncellendi:', route.params.constants);
    }
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (connected) {
        console.log('⚙️ Motor değerleri hesaplandı, BLE üzerinden gönderiliyor...');
        BLEService.sendJoystickData(newMotorValues)
          .then(success => {
            if (success) {
              console.log('✅ Joystick verisi başarıyla BLE cihazına gönderildi');
            } else {
              console.log('❌ Joystick verisi gönderilemedi');
            }
          })
          .catch(error => {
            console.log('❌ Joystick veri gönderim hatası:', error);
          });
      }
    } catch (error) {
